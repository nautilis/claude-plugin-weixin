...
| 接收图片、视频和文件 | 自动下载、解密并落到本地 inbox，Claude 直接读文件（视频只落盘，看不了内容） |
...
入站的照片、视频和文件附件会被下载、解密并写入 `~/.claude/channels/weixin/inbox/`。通道通知里带
`image_path`（第一张照片）和 `attachments`（全部附件的 JSON），Claude 从那里读文件。
超过 100MB 的附件会被跳过，超过 7 天的文件在 server 启动时清理。语音消息目前不下载。

视频以 `kind: "video"` 出现在 `attachments` 里，落盘为 `.mp4` —— 视频项不带文件名，微信发的就是
mp4。Claude 读不了视频内容，只知道来了一个视频，文件放在那里等你自己处理。发送视频暂不支持。
