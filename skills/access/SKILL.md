...
`<state-dir>/access.json`. You never talk to WeChat — you just
...
a second bot points that variable elsewhere, and editing the default dir would
silently change the wrong bot's allowlist.
...
7. `mkdir -p <state-dir>/approved` then write
   `<state-dir>/approved/<senderId>` with empty content.
